...
export default function VolunteerEventDetailPage() {
  ...
  return <div><button onClick={() => router.back()} className="mb-5 inline-flex items-center gap-2 text-sm text-mist hover:text-ink"><ArrowLeft className="h-4 w-4" /> Back to events</button><PageHeader title={event.title} description={event.description || "Community response event."} actions={<Button onClick={toggle} disabled={busy || event.status === "CLOSED" || event.status === "CANCELLED"}>{busy ? "Saving…" : event.joinedByCurrentVolunteer ? "Withdraw" : "Join event"}</Button>} /><div className="flex flex-wrap gap-2 mb-6"><EventStatusBadge status={event.status} /><EventTypeBadge type={event.type} /><SeverityBadge severity={event.severity} /></div><div className="grid grid-cols-2 md:grid-cols-3 gap-4 mb-8"><StatCard label="People joined" value={event.participantCount} hint={`${event.requiredVolunteers} needed`} /><StatCard label="Organizer" value={event.organizerName} hint={event.organizerType} /><StatCard label="Your status" value={event.joinedByCurrentVolunteer ? "Joined" : "Not joined"} hint="You can withdraw any time" /></div><div className="nx-card space-y-4 text-sm text-mist"><div className="flex items-center gap-3"><MapPin className="h-4 w-4 text-signal" />{event.divisions.map((d) => d.name).join(", ") || "Bangladesh"}</div><div className="flex items-center gap-3"><CalendarDays className="h-4 w-4 text-signal" />{formatDateTime(event.startAt)} → {formatDateTime(event.endAt)}</div><div className="flex items-center gap-3"><Users className="h-4 w-4 text-signal" />{event.participantCount} volunteers have joined this response.</div></div></div>;
}
